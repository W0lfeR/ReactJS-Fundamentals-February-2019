export const AJAX_BEGIN = 'AJAX_BEGIN'
export const AJAX_END = 'AJAX_END'
export const REDIRECTED = 'REDIRECTED'
export const REGISTER_SUCCESS = 'REGISTER_SUCCESS'
export const LOGIN_SUCCESS = 'LOGIN_SUCCESS'
export const REGISTER_ERROR = 'REGISTER_ERROR'
export const LOGIN_ERROR = 'LOGIN_ERROR'
export const LOGOUT_SUCCESS = 'LOGOUT_SUCCESS'
export const FETCH_DATA_SUCCESS = 'FETCH_DATA_SUCCESS'
export const CREATE_COMPUTER_SUCCESS = 'CREATE_COMPUTER_SUCCESS'
export const CREATE_COMPUTER_ERROR = 'CREATE_COMPUTER_ERROR'
export const EDIT_COMPUTER_SUCCESS = 'EDIT_COMPUTER_SUCCESS'
export const EDIT_COMPUTER_ERROR = 'EDIT_COMPUTER_ERROR'
export const DELETE_COMPUTER = 'DELETE_COMPUTER'
export const FETCH_STATS_SUCCESS = 'FETCH_STATS_SUCCESS'
export const ADD_TO_CART = 'ADD_TO_CART'
export const SYNC_CART = 'SYNC_CART'
export const REMOVE_FROM_CART = 'REMOVE_FROM_CART'
export const SUBMIT_ORDER = 'SUBMIT_ORDER'
export const FETCH_USER_ORDERS = 'FETCH_USER_ORDERS'
export const FETCH_PENDING_ORDERS = 'FETCH_PENDING_ORDERS'
export const APPROVED_ORDER = 'APPROVED_ORDER'
