...
async function fetchProducts () {
  const res = await window.fetch(host + 'computer/all')
  return res.json()
}

async function createProduct (data) {
  const res = await window.fetch(host + 'computer/create', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
...
async function editProduct (id, data) {
  const res = await window.fetch(host + `computer/edit/${id}`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': 'bearer ' + Auth.getToken()
...
async function deleteProduct (id) {
  const res = await window.fetch(host + `computer/delete/${id}`, {
    method: 'DELETE',
    headers: {
      'Authorization': 'bearer ' + Auth.getToken()
    }
